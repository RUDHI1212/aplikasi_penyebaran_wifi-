                        ...
                        if( ! empty($db)){ // Jika data pada database tidak sama dengan empty (alias ada datanya)
                          foreach($db as $data){ // Lakukan looping pada variabel db dari controller
                            ...
                            echo "<td class='text-center'>
                                  <a href='#' data-toggle='modal' data-target='#ubahbmt".$data->id_user."'><button type='button' class='btn btn-primary btn-circle btn-sm'><i class='fas fa-edit'></i></button></a>

                                    <a href='#' data-toggle='modal' data-target='#hapusbmt".$data->id_user."'><button type='button' class='btn btn-danger btn-circle btn-sm'><i class='fas fa-trash'></i></button></a>
                                  </td>
                            ";
...
<?php foreach ($db as $data) { ?>
  <div class="modal fade" id="hapusbmt<?= $data->id_user;?>" tabindex="-1" role="dialog" aria-labelledatay="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Apa benar anda ingin menghapus BMT?</h5>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
        </div>
        <div class="modal-body">Klik hapus untuk menghapus BMT </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
          <a class="btn btn-primary" href="<?php echo base_url('ADMIN/home/hapusbmt/'.$data->id_user); ?>"> Hapus</a>
        </div>
      </div>
    </div>
  </div>
<?php }?>
 ...
 <!-- Ubah Modal-->
<?php foreach ($db as $data) { ?>
   <div class="modal fade" id="ubahbmt<?= $data->id_user;?>" tabindex="-1" role="dialog" aria-labelledatay="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <?php echo form_open('ADMIN/home/updateBMT/'.$data->id_user, ['method' => 'post']); ?>
